...
class Grid {
	...
	void draw(Graphics2D g, int w, int h) {
        ...
        if (computerMoves.size() > 0) {
        	for (Map.Entry<Point, Integer> mapEntry : computerMoves.entrySet()) {
            	g.setFont(new Font("TimesRoman", Font.PLAIN, 15));
            	g.setColor(Color.red);
            	g.drawString(mapEntry.getValue().toString(), origX + mapEntry.getKey().x * cellSize, origY + mapEntry.getKey().y * cellSize - 5);
            }
        }
        
        // playerMoves
        if (playerMoves.size() > 0) {
        	for (Map.Entry<Point, Integer> mapEntry : playerMoves.entrySet()) {
            	g.setFont(new Font("TimesRoman", Font.PLAIN, 15));
            	g.setColor(Color.blue);
            	g.drawString(mapEntry.getValue().toString(), origX + mapEntry.getKey().x * cellSize, origY + mapEntry.getKey().y * cellSize - 5);
            }
    ...
    private void drawPoint(Graphics2D g, int x, int y) {
        ...
        if(computerMoves.containsKey(new Point(x, y)) || playerMoves.containsKey(new Point(x, y))) {
